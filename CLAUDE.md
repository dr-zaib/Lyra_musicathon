...
- ⚠️ **Axel — `language` plumbing needed (frontend is ready)**: the frontend now sends a **`language`** field (ISO 639-1, default = the browser's, user-overridable in Settings) on `/entry` `/journey` `/refill` `/agent`, mirrored in `web/src/lib/types.ts`. To make it live: add `language: str | None = None` to `AgentTurnRequest`/`EntryRequest`/`JourneyRequest`/`RefillRequest` in `shared/schema.py`, thread it in `backend/app.py`, and pass `lang=language` into `_fetch_candidates(...)` in `engine/trajectory.py` (today hardcoded `"en"`). **Heads-up**: if those Pydantic models forbid extra fields, the new field will 400 and the frontend silently falls back to mock — so either add the field or confirm extras are ignored.
