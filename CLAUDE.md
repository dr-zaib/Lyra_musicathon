...
- **→ Agent contract additions (OPEN — pending Alberto+Axel sync, not yet locked)**: the frontend needs `confidence: float (0..1)` and an emotion `distribution` from the agent/`/recommend` to drive the comprehension bar + wheel. Three questions to resolve together (then update `shared/schema.py` ↔ `web/src/lib/types.ts` in lockstep):
  1. **Where does `confidence` live** — top-level of the `/recommend` response, or inside `Trajectory`?
  2. **Which `distribution` drives the wheel** — the agent's read of the *user's* mood (intent), or the in-playback step's `target_distribution`? (Two different things; the mock currently conflates them.)
  3. **How does free text enter the contract** — add a `message` field to `TrajectoryRequest`, or a separate `/interpret` endpoint (text → seed/distribution + confidence)?
